...
def download_league_data(url):
    ...
    league_data = league_data[['HomeTeam', 'AwayTeam', 'HTHG', 'HTAG']]
...
def historyfunc(path, hw, aw):
    ...
    for year in range (1, 5):
        ...
        try:
            if (ht_found.loc[ht_found["AwayTeam"] == aw]['HTHG'].iloc[0]) > (ht_found.loc[ht_found["AwayTeam"] == aw]['HTAG'].iloc[0]):
                win = win + 1
            elif (ht_found.loc[ht_found["AwayTeam"] == aw]['HTHG'].iloc[0]) < (ht_found.loc[ht_found["AwayTeam"] == aw]['HTAG'].iloc[0]):
                lose = lose + 1
            elif (ht_found.loc[ht_found["AwayTeam"] == aw]['HTHG'].iloc[0]) == (ht_found.loc[ht_found["AwayTeam"] == aw]['HTAG'].iloc[0]):
                draw = draw + 1

            if (ht_found.loc[ht_found["AwayTeam"] == aw]['HTHG'].iloc[0]) + (ht_found.loc[ht_found["AwayTeam"] == aw]['HTAG'].iloc[0]) > 1:
                ov0_5 = ov0_5 + 1
    ...
    if win + draw + lose > 0:
        ...
        perc_d = draw/totalm
        perc_a = lose/totalm
        perc_ov0_5 = ov0_5/totalm
        perc_ov1_5 = ov1_5 / totalm
        perc_ov2_5 = ov2_5 / totalm
        perc_gg = gg / totalm
    else:
        totalm = 0
        perc_h = 0
    ...
    return(perc_h, perc_d, perc_a, perc_ov0_5,perc_ov1_5,perc_ov2_5,perc_gg)
...
def save_results(lista, listb):
    ...
    path_txt = F"D:/Python Apps/Betting/History_{YEAR}/" + weekend + F"_HT_{YEAR}.txt"
...
def save_results_excel(lista, listb):
    ...
    sheet_P = wb["HT_Poison"]
    sheet_H = wb["HT_+History"]
...
def perteam_poisson(leagudata, ht, at):
    # predict number of goals poison model
    matchdata = leagudata.loc[(leagudata['HomeTeam']==ht) | (leagudata['AwayTeam']==at)]
    #print(matchdata)
    poisson_pred = np.column_stack([[poisson.pmf(i, matchdata.mean()[j]) for i in range(8)] for j in range(2)])
    dt_home = matchdata[['HomeTeam', 'AwayTeam', 'HomeGoals']].assign(home=1).rename(
        columns={'HomeTeam': 'team', 'AwayTeam': 'opponent', 'HomeGoals': 'goals'})
    dt_away = matchdata[['AwayTeam', 'HomeTeam', 'AwayGoals']].assign(home=0).rename(
            columns={'AwayTeam': 'team', 'HomeTeam': 'opponent', 'AwayGoals': 'goals'})

    goal_model_data = pd.concat([dt_home,dt_away])
    poisson_model = smf.glm(formula="goals ~ home + team + opponent", data=goal_model_data,
                            family=sm.families.Poisson()).fit()
    poisson_model.summary()
    return(poisson_model)

def league_poisson(leagudata):
    poisson_pred = np.column_stack([[poisson.pmf(i, leagudata.mean()[j]) for i in range(8)] for j in range(2)])
    dt_home = leagudata[['HomeTeam', 'AwayTeam', 'HomeGoals']].assign(home=1).rename(
        columns={'HomeTeam': 'team', 'AwayTeam': 'opponent', 'HomeGoals': 'goals'})
    dt_away = leagudata[['AwayTeam', 'HomeTeam', 'AwayGoals']].assign(home=0).rename(
            columns={'AwayTeam': 'team', 'HomeTeam': 'opponent', 'AwayGoals': 'goals'})
...
if __name__ == '__main__':
    ...
    for i in (ran):
        ...
        for match in next_match.loc[next_match['Div']==divis].index:
            ...
            try:
                poisson_model = perteam_poisson(league_data,ht,at)
            except:
                print('not enough data.. skipping')
                continue

            htg = league_data.loc[league_data['HomeTeam'] == ht, 'HomeGoals']
            atg = league_data.loc[league_data['AwayTeam'] == at, 'AwayGoals']
            temp = max(htg.mean(), atg.mean())
            temph = int(round(temp / 2))

            maxg = int(round(temp))+temph
            if maxg < 3:
                maxg = 4
            try:
                result = simulate_match(poisson_model, ht, at, maxg)
            except:
                continue
            ...
            if (np.sum(np.tril(result, -1))> 0.98 or np.sum(np.triu(result, 1)) > 0.98 or
                    np.sum(np.diag(result)) > 0.98 or under2_5 > 0.98 or over2_5 > 0.98):
                continue
            ...
            if np.sum(np.tril(result, -1)) > 0.7:
                ...
                a,b,c,o5,o15,o25,gg = historyfunc(path,ht,at)
                if a or b or c > 0:
            ...
            if over0_5 > 0.8:
                result_play.append(
                    [next_match['Div'][match], dt, tm, ht + " - " + at, "O0.5", str(over0_5 * 100)[:5] + "%"])

                a,b,c,o5,o15,o25,gg = historyfunc(path,ht,at)
                if a or b or c > 0:
                    weight_av = 0.85 * (over0_5) + 0.15 * (o5)
                    if weight_av > 0.8:
                        my_res.append([next_match['Div'][match], dt, tm, ht + " - " + at, "O0.5",
                                       str(weight_av * 100)[:5] + "%"])

            # over 1.5
            if over1_5 > 0.7:
                result_play.append([next_match['Div'][match], dt ,tm ,ht + " - " + at , "O1.5", str(over1_5 * 100)[:5] + "%"])

                a,b,c,o5,o15,o25,gg = historyfunc(path,ht,at)
                if a or b or c > 0:
                    weight_av = 0.85 * (over1_5) + 0.15 * (o15)
                    if weight_av > 0.7:
                        my_res.append([next_match['Div'][match], dt , tm , ht + " - " + at , "O1.5", str(weight_av * 100)[:5] + "%"])

            # over 2.5
            if over2_5 > 0.7:
                result_play.append([next_match['Div'][match], dt ,tm ,ht + " - " + at , "O2.5", str(over2_5 * 100)[:5] + "%"])

                a,b,c,o5,o15,o25,gg = historyfunc(path,ht,at)
                if a or b or c > 0:
                    weight_av = 0.85 * (over2_5) + 0.15 * (o25)
                    if weight_av > 0.7:
                        my_res.append([next_match['Div'][match], dt , tm , ht + " - " + at , "O2.5", str(weight_av * 100)[:5] + "%"])
            ...
            if np.sum(goalgoal) > 0.7:
                ...
                a,b,c,o5,o15,o25,gg = historyfunc(path, ht, at)
                if a or b or c > 0:
                    weight_av = 0.85 * (np.sum(goalgoal)) + 0.15 * (gg)
                    if weight_av > 0.7:
