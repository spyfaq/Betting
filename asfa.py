...
"""
    ...
    def_league function with current year
        2021 to next etc...
    historyfunc function with current year
...
def save_results(lista, listb):
    ...
    path_txt = f"D:/Python Apps/Betting/History_{YEAR}/" + weekend + f"dynamic_{YEAR}.txt"
...
def save_results_excel(lista, listb):
    ...
    sheet_P = wb["FT_Dynamic"]
    sheet_H = wb["FT_D+History"]
...
def perteam_poisson(matchdata):
    ...
    poisson_model = smf.glm(formula="home + team + opponent ~ goals ", data=goal_model_data,
...
if __name__ == '__main__':
    ...
    for i in (ran):
        ...
        # predict number of goals poison model-- not used
        ...
        leagumodel = league_poisson(league_data)
        ...
        for match in next_match.loc[next_match['Div']==divis].index:
            ...
            matchdata = league_data.loc[(league_data['HomeTeam']==ht) | (league_data['AwayTeam']==at)]
            try:
                poisson_model = perteam_poisson(matchdata)
            except:
            ...
            try:
                result_t = simulate_match(poisson_model, ht, at, maxg)
                result_l = simulate_match(leagumodel, ht, at, maxg)
                result = result_t * 0.7 + result_l * 0.3
            except:
                continue


            under3_5 = result[0][0] + result[0][1] + result[0][2] + result[1][2] + result[0][3] + result[1][0] + result[1][1] + result[2][0] + result[2][1] + result[3][0]
            ...
            # under 2.5
            if under2_5 > 0.7:
                result_play.append([next_match['Div'][match], dt , tm , ht + " - " + at , "U",  str(under2_5 * 100)[:5] + "%"])

                a,b,c,d,e,g = historyfunc(path,ht,at)
                if a or b or c > 0:
                    weight_av = 0.75 * (under2_5) + 0.25 * (1-d)
                    if weight_av > 0.75:
                        my_res.append([next_match['Div'][match], dt , tm , ht + " - " + at , "U", str(weight_av * 100)[:5] + "%"])

            # over 2.5
            if over2_5 > 0.7:
                result_play.append([next_match['Div'][match], dt ,tm ,ht + " - " + at , "O", str(over2_5 * 100)[:5] + "%"])

                a,b,c,d,e,g = historyfunc(path,ht,at)
                if a or b or c > 0:
                    weight_av = 0.85 * (over2_5) + 0.15 * (d)
                    if weight_av > 0.7:
